...
Byte Core has no functional release or supported installed command-line interface yet. The internal bootstrap implements `byte check`, plan-bound deployment initialization, a preservation-only removal check, and read-only installation-manifest planning proofs. Do not claim that installation, activation, destructive removal, update, diagnostics, shell integration, or other planned lifecycle behavior is implemented.
